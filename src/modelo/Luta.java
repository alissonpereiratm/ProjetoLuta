...
public class Luta {
	 ...
	 * 
	 * public Luta marcarLuta(ArrayList<Lutador> lutadores) {
		Luta luta = null;
		String nomes = "";
		...
		if (lutadores.size() > 1) {
			String primeiroLutador = JOptionPane.showInputDialog("Digite o nome do primeiro lutador" + nomes);
			for (Lutador lutador : lutadores) {
				if (lutador.getNome().equalsIgnoreCase(primeiroLutador)) {
					this.lutador1 = lutador.getNome();
					String segundoLutador = JOptionPane.showInputDialog("Digite o nome do segundo lutador" + nomes);
					for (Lutador lutador2 : lutadores) {
						if (lutador2.getNome().equalsIgnoreCase(segundoLutador)) {
							if ((lutador.getCategoria().equalsIgnoreCase(lutador2.getCategoria()))
									&& (!lutador.getNome().equalsIgnoreCase(lutador2.getNome()))) {
								this.lutador2 = lutador2.getNome();
								this.round = Integer
										.parseInt(JOptionPane.showInputDialog(null, "Digite a quantidade de rounds"));

								JOptionPane.showMessageDialog(null, "A luta entre " + lutador.getNome() + " e "
										+ lutador2.getNome() + " foi marcada!");
								luta = new Luta(this.round, this.lutador1, this.lutador2);
								return luta;
							} else {
			...
			JOptionPane.showMessageDialog(null, "Não há lutadores o suficiente para marcar uma luta");
		return null;
	}*/
